...
def main():
    ...
    pattern = '''    if !is_hnsw_index_loaded() || !is_active_hnsw_collection(&collection_id) {
        debug!(
            "[search_chunks] HNSW missing or different collection active. Rebuilding for {}",
            collection_id
        );
    ...
    new_code = '''    if !is_hnsw_index_loaded() || !is_active_hnsw_collection(&collection_id) {
        debug!(
    ...
    if pattern in content:
        ...
        print("Could not find the search_chunks block to replace")
        sys.exit(1)
